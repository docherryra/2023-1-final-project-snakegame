...
void Snake::snakeDirection(int d){ //스네이크의 방향을 설정하는 함수 구현
	switch(d)
	{
	...
	for(int i=0; i<wall.size(); i++) { //벽의 벡터만큼 for문
		if(snake_vec[0] == wall[i]) { //벽일경우
			if(snake_vec[0] == gate[1]) { //gate[1]과만나면
				...
				snakeDirection(gateDirection(gate[0], map)); //snake의 head 의 방향 바꾸어줌
				setGateCnt(1);
				break;
			}
				...
				snakeDirection(gateDirection(gate[1], map));
				setGateCnt(1);
				break;
			}
...
void Snake::reduceItem(WINDOW *win1){
	removeRock(level-1,win1);
	snake_vec.pop_back();
	makeMango(level-1,win1);
}

void Snake::changeItem(WINDOW *win1){
	removeMango(level -1,win1);
	snake_vec.push_back(snake_vec.back());
	makeRock(level-1,win1);
}

position Snake::SnakeHead(){ //머리 position 타입으로 바꿔주기
